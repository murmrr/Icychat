export const idlFactory = ({ IDL }) => {
  ...
  const Result_6 = IDL.Variant({ ok: IDL.Null, err: CreateChatError });
  ...
  const GetMyChatsError = IDL.Variant({ UserNotFound: IDL.Null });
  const Result_5 = IDL.Variant({
    ok: IDL.Vec(SharedChat),
    err: GetMyChatsError,
  });
  ...
  const Result_1 = IDL.Variant({ ok: IDL.Null, err: SendMessageError });
  const UpdateProfileError = IDL.Variant({ ProfileNotFound: IDL.Null });
  const Result = IDL.Variant({ ok: IDL.Null, err: UpdateProfileError });
  return IDL.Service({
    createChat: IDL.Func([IDL.Principal], [Result_6], []),
    getMyChats: IDL.Func([], [Result_5], ["query"]),
    ...
    sendMessage: IDL.Func([IDL.Principal, MessageContent], [Result_1], []),
