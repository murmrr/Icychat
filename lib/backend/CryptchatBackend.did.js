export const idlFactory = ({ IDL }) => {
  ...
  const Result_11 = IDL.Variant({ ok: IDL.Null, err: AddToChatError });
  const BurnAccountError = IDL.Variant({ UserNotFound: IDL.Null });
  const Result_10 = IDL.Variant({ ok: IDL.Null, err: BurnAccountError });
  const CreateChatError = IDL.Variant({
    ChatAlreadyExists: IDL.Null,
    UserNotFound: IDL.Null,
  });
  const Result_9 = IDL.Variant({ ok: IDL.Null, err: CreateChatError });
  ...
  const Result_8 = IDL.Variant({ ok: SharedChat, err: GetMyChatError });
  ...
  const Result_6 = IDL.Variant({ ok: Profile, err: GetMyProfileError });
  const GetProfileError = IDL.Variant({ UserNotFound: IDL.Null });
  const Result_5 = IDL.Variant({ ok: Profile, err: GetProfileError });
  const GetPublicKeyError = IDL.Variant({ UserNotFound: IDL.Null });
  const Result_4 = IDL.Variant({ ok: IDL.Text, err: GetPublicKeyError });
  ...
  return IDL.Service({
    addToChat: IDL.Func([IDL.Nat, IDL.Principal, IDL.Text], [Result_11], []),
    burnAccount: IDL.Func([], [Result_10], []),
    createChat: IDL.Func([IDL.Principal, IDL.Text, IDL.Text], [Result_9], []),
    getMyChat: IDL.Func([IDL.Nat], [Result_8], ["query"]),
    getMyChatHeaders: IDL.Func([], [Result_7], ["query"]),
    getMyProfile: IDL.Func([], [Result_6], ["query"]),
    getProfile: IDL.Func([IDL.Principal], [Result_5], ["query"]),
    getPublicKey: IDL.Func([IDL.Principal], [Result_4], ["query"]),
